<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
    ...
    while($ar_result = $db_list->GetNext())
    {
        ...
        if ($ar_result['CODE'] === $sectionCode) {
            ...
            foreach ($ar_result['UF_FILTERS'] as $filter) {
                $arrFilter = explode('/',$filter);
                $filters[$arrFilter[0]] = ['NAME' => $arrFilter[2] , 'TYPE' => $arrFilter[1]];
            }
        ...
        <div class="catalog-selector__body">
            <div class="catalog-selector__wrapper">
                <ul class="catalog-selector">
                    <li><a href="/programs/" class="catalog-selector__item">Все</a></li>
                    <? foreach ($sections as $SECTION):?>
                        <li><a href="<?=$SECTION['SECTION_PAGE_URL']?>" class="catalog-selector__item <?=$SECTION['ACTIVE']?>"><?=$SECTION['NAME']?></a>
                        </li>
                    <? endforeach;?>
                </ul>
            </div>
